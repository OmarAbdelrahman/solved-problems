...
int main(int argc, char** argv) {
  long long n = io::next<long long>();
  long long m = io::next<long long>();
  if (m == 1) {
    const long long result = (n * (n - 1)) / 2;
    ...
    long long each = n / m;
    long long rem = n % m;
    long long without_rem = m - rem;
    long long each_with_rem = each + 1;
    long long kmin = ((each * (each - 1)) / 2) * without_rem;
    each += 1;
    kmin += ((each * (each - 1)) / 2) * rem;
    cout << kmin << " " << kmax << endl;
  }
  return 0;
}
