...
int main(int argc, char** argv) {
  int n = io::next<int>();
  int road_len = io::next<int>();
  vector<double> lanterns = io::next_array<double>(n);

  sort(lanterns.begin(), lanterns.end());
  
  double max_dist = 0;
  for (int i = 0; i + 1 < lanterns.size(); ++i) {
    max_dist = max(max_dist, lanterns[i + 1] - lanterns[i]);
  }
  
  cout << fixed << max(max_dist / 2.0, max(lanterns[0], road_len - lanterns.back())) << endl;
  return 0;
}
