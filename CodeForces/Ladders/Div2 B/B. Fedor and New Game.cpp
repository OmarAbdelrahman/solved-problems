...
int bit_count(int n) {
  return n == 0 ? 0 : 1 + bit_count(n & (n - 1));
}

int main(int argc, char** argv) {
  int types = io::next<int>();
  int soldiers = io::next<int>();
  int k = io::next<int>();
  
  vector<int> players = io::next_array<int>(soldiers);
  int fedor = io::next<int>();

  cout << count_if(players.begin(), players.end(), [&](const int player) { return bit_count(fedor ^ player) <= k; }) << endl;
  return 0;
}
